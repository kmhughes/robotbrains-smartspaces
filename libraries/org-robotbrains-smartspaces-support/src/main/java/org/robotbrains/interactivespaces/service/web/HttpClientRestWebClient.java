...
package org.robotbrains.interactivespaces.service.web;
        ...
        if (statusCode == HttpStatus.SC_OK) {
          if (entity != null) {
            ...
            try {
              ...
              throw new SmartSpacesException(String.format("Exception during REST call %s",
                  request.getURI()), e);
            } finally {
          ...
          throw new SimpleSmartSpacesException(String.format(
              "Server returned bad status code %d for REST call to URI %s", statusCode,
              request.getURI()));
        }
      } catch (SmartSpacesException e) {
        throw e;
      } catch (Exception e) {
        throw new SmartSpacesException(String.format("Could not read REST URI %s",
            request.getURI()), e);
      } finally {
        if (entity != null) {
          try {
            ...
            throw new SmartSpacesException(String.format(
                "Could not consume entity content for REST call %s", request.getURI()), e);
          }
        }
      }
    }
